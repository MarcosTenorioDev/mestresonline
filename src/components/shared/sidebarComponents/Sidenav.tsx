import { type NavItem } from "@/core/interfaces/navbarTypes";
...
import {
	...
	AccordionItem,
	AccordionTrigger,
} from "@/components/shared/sidebarComponents/Subnav-accordion";
...
export function SideNav({ items, setOpen, className }: SideNavProps) {
  ...
  const params = useParams()
	...
	return (
		<nav className={`${isOpen && 'space-y-2 px-3'}`}>
			{items.map((item) =>
				item.isChidren ? (
					<Accordion
						type="single"
					...
					<Link
						key={item.title}
						to={item.protected ? `${item.href}/${params.id}` : item.href}
						onClick={() => {
							if (setOpen) setOpen(false);
						}}
